...
public class StudentService {
	...
	// 하지만 리스트의 크기는 필요에따라 늘었다 줄었다 하기 때문에 큰 의미 없음
	...
	public StudentService() {	// 기본 생성자 (5명 만들어두기)
	
		studentList.add(new Student("홍길동", 23, "서울시 중구", 'M', 100));
		studentList.add(new Student("고영희", 30, "경기도 안산시", 'F', 100));
		studentList.add(new Student("강아지", 27, "서울시 강남구", 'M', 80));
		studentList.add(new Student("오미나", 22, "충북 청주시", 'F', 90));
		studentList.add(new Student("박주희", 20, "서울시 서대문구", 'F', 70));
	...
	public void selectAll() {
		...
		if(studentList.isEmpty()) {
			...
			return;	// 현재메서드(selectAll)를 종료하고 호출한 곳으로 돌아감.
		}
		
		// 일반 for 문
		/*
		for(int i = 0; i < studentList.size(); i++) { // length XX
	 ...
	 *   
	 *   3. 수정이 완료되었다면, "000의 정보가 변경되었씁니다" 반환
	 * @return
	 */
	public String updateStudent() {
		...
		// 3) studentList 범위 내 인덱스 번호인지 검사,
			...
			// index번째에 있던 기존 학생 정보가 반환되고, 그 객체를 temp에 저장 (기존학생 정보)
	 ...
	 * Student 객체의 name값에 포함되는 문자열인지 검사
	 * 
	 ...
	 * List를 정렬하는 방법 2가지
	 * 
	 * 1. Comparable 인터페이스 상복받아 comparaTo() 메소드 재정의
	 * 	  Student에 Comparable 인터페이스를 상속받아 오버라이딩한 comparaTo()에
	...
	public void sortByName() {
									// 익명 내부클래스는 Comparator 인터페이스를 
									// 상속받아 구현한 구현체 (== 클래스)
		Collections.sort(studentList, new Comparator<Student>() {
									// = public void ABC implements Comparator
			@Override
			public int compare(Student o1, Student o2) {
				// 이름 비교
				return o1.getName().compareTo(o2.getName());
				// name은 String형(참조형)이라 compareTo로 비교
				// 내림차순 하려면 o1, o2 순서 바꾸면 됨
				
				// String.compareTo() : 자바에서 객체를 비교하는 메서드
				// (String이 Comparable을 상속받아 재정의해둔 compareTo() 메서드를 이용하는 것)
				
				// compareTo() : 두 객체를 비교하고 순서 결정함.
				// 반환값 : 0(같음), 양수(왼쪽개체가 더 큼), 음수(왼쪽객체가 더 작음)
			}	
