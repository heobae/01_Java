...
//	   속성 (필드)
	...
	// 기본생성자 자동완성 단축키 : ctrl + spacebar + enter
	// 기본생성자 ( -> 매개변수 없는 생성자)
	public User() {
		// 기능
	...
	// alt + shift + s -> 매개변수 생성자 자동완성
	// ** 매개변수 : 생성자나 메서드 호출 시 () 안에 작성되어
	//				전달되어지는 값을 저장하고 있는 변수
	//				-> 전달받은 값을 저장하고 있는 매개체 역할의 변수
	public User(String userId, String userPw) { // 오버로딩
		System.out.println("매개변수 생성자를 2개 이용하여 User 객체 생성");
	...
	public User( String userId, String userPw, String userName,
		...
		// 유의사항 : 생성자 내에서 반드시 첫번째 줄에 작성!
		// 왜 사용? -> 중복 코드 제거, 코드길이 감소, 재사용성 증가
		// (단점) 가독성이 떨어짐
	...
	// public User() {} // 기본생성자가 이미 작성되어 있어서 X
			...
			// 매개변수의 개수가 동일한 것은 있으나, 하나의 타입이 다름
			// -> 오버로딩 성립
			
			//public User(int userAge, String userId) {}
			// 매개변수의 개수와 타입이 동일하지만, 순서가 다름
			
			//public User(int userAge, String userName) {}
			// 위 매개변수의 개수, 타입, 순서 모두 같아서 불가
			// -> 변수명은 신경쓰지 않는다
	...
	// toString() 메서드 : Object 최상위 클래스
	// 이미 만들어져있는 메서드 객체가 문자열로 변환될 때 호출되는 메서드
