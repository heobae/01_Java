...
public class Array2Example {
	...
	*/	
		...
		// arr[0]이 참조하고 있는 배열의 길이     (메모리구조 pdf참고)
		...
		// 3) 선언과 동시에 초기화
		int[][] arr2 = {{10, 20, 30}, {40, 50, 60}}; // 2행 3열
		
		// Arrays.toString(배열명) : 참조하고 있는 1차원 배열의 값을 문자열로 반환 (1행까지만 보여줌)
	...
	public void ex2() {
		...
		for(int row = 0; row < arr.length; row++) { // 행 반복 ( arr.length = (3) )
			
			int sum = 0;
			
			for(int col = 0; col < arr[row].length; col++) { // 열 반복 ( arr[row].length = 3 )
	...
	public void ex3() {
		// 가변 배열
		// - 2차원 배열 생성 시 마지막 배열 차수(열)를 지정하지 않고 -> ([] 빈공간으로)
		//   나중에 서로 크기가 다른 1차원 배열을 생성하여 참조하는 배열
		
		char[][] arr = new char[4][];
		// char 2차원 배열 생성 시 행 부분만 생성
		
		arr[0] = new char[3]; // new char[4] -> 0행에 3열짜리 1차원 배열을 생성하여 주소값 저장
