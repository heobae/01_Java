...
public class ByteService {
	...
	// -> IOException 예외 처리 구문을 작성하는 것이 필수! (Checked Exception이니까)
	...
	public void fileByteOutput() {
		...
		FileOutputStream fos = null;	// finally에서 fos에 다가갈 수 있게 전역변수로 만들어 둠 
		...
		try {
			...
			*/
			...
			// ***** 수행 시간 확인 *****
			// write() 수행되는 시간 측정
			// System.nanoTime() : 1970.01.01 오전 09:00:00 부터
			...
			fos.flush();  // 스트림 안에 남아있는 모든 데이터를 모두 밀어냄 (선택사항)
			// FileOutputStream에서는 flush() 선택사항
	...
	public void bufferedFileByteOutput() {
		...
		try {
			...
			bos.write(content.getBytes());	// 버퍼에 저장(파일에 즉시 기록X)
			
			long endTime = System.nanoTime();
			
			System.out.println("[수행시간] : " + (endTime - startTime) + "ns");
			
			bos.flush();	// 스트림 안에 남아있는 모든 데이터를 밀어냄 (기본스트림과 다르게 필수!!!)
			// 버퍼 내용을 강제로 파일에 기록
			// BufferedOutputStream 사용 시 flush() 필수!!
			// finally 구문에 close() 쓰게 되면 안 써도 됨
	...
	public void fileByteInput() {
		...
		try {
			...
			// byte -> 자바에서 정수형
			// 다루기가 힘들기 때문에 정수형 기본형인 int로 변환하여 사용
			...
			while(true) {
				...
				value = fis.read();	// 1byte씩 끊어서 읽어오는 메서드, 정수값으로 변환
				// 단, 더이상 읽어올 값이 없으면 -1 반환
				
				if(value == -1) break;	// 다 읽어왔으면 반복 멈추겠다(while문 빠져 나오겠다)
				
				sb.append( (char)value);	// char로 강제형변환 해서 글자 형태로 sb 추가
			}
	...
	public void bufferedFileByteInput() {
		...
		try {
			...
			try {
				// 보조 스트림인 close()해도 기반 스트림도 같이 close() 됨
	...
	public void readProperties() {
		...
		try {
			...
			System.out.println("language : " + properties.getProperty(""));	// 없는 값 지정하면 null 반환
	 ...
	 * 	  : FileOutputStream + BufferedOutput
	 * 
	 * + Scanner 대신 BufferedReader (문자열 입력용 스트림) 이용
	 * -> BufferedReader는 보조스트림으로 단독사용 불가능!
	 * -> BufferedReader 생성 시 기반스트림인 InputStreamReader -> InputStream이 기반
	 */
	public void fileCopy() {
		...
		try {
			...
			br = new BufferedReader(new InputStreamReader(System.in)); // System.in -> InputStreamReader 문자 스트림 변환
															// 바이트 스트림
			...
			// 입력용 스트림 생성 (외부 -> 내부니까 입력)
			fis = new FileInputStream(target);
			bis = new BufferedInputStream(fis);
			
			// 입력용 스트림을 이용해서 target 파일의 내용 입력받기
			byte[] bytes = bis.readAllBytes();	// bytes에 메모장 노래가사가 저장된 상황
			...
			try {
				if(br != null) br.close();
				if(bis != null) bis.close(); // 기반인 fis도 닫힘
				if(bos != null) bos.close(); // 기반인 fos도 닫힘
