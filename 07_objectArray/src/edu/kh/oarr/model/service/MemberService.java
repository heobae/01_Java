...
public class MemberService {
	...
	// 회원가입한 사람의 정보를 저장해 둘 참조변수		
	// Member 5칸짜리 객체배열 선언 및 할당
	// 변수값을 입력하기 전에 JVM초기값 (null)이 입력됨
	// 참조변수라서 null
	...
	public int emptyIndex() {   // int인 이유는 빈공간의 인덱스 번호를 얻어야해서
		...
		// for문을 수행했지만 return이 되지 않은 경우 해당위치 코드가 수행된다!
	...
	public void searchRegion() {
		...
		for(int i = 0; i < memberArr.length; i++) {
			...
			//    입력 받은 지역(inputRegion)과 같을 경우
			//    memeberArr[i] 요소에 저장된 객체의
			//    회원의 아이디, 이름을 출력
	...
	public String selectMember() {
		...
		// 1) 로그인 여부 확인
		// 2-> 필드 loginMember가 참조하는 객체가 있는지 확인
		if(loginMember == null) {
			return "로그인 후 이용바람";
		}
		// 2) 로그인이 되어있는 경우
	...
	public int updateMember() {
		...
		//    1 반환
			...
			if(inputMemberPw.equals(loginMember.getMemberPw())) {
				
				loginMember.setMemberName(inputMemberPw);
