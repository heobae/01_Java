...
//             ㄴ 참조 변수
		...
		// 배열의 인덱스 범위를 벗어남 : 길이가 4칸까지 배열인데, 없는 인덱스번호인 4에 접근했다.
	...
	public void ex4() {
		...
		int sum = 0; // 합계 저장용 변수(강사님이 하신 부분)
		...
		int min = score[0];
			...
			for(int i = 0; i < score.length; i++) {
				...
				// 최고점 비교
				if(score[i] > max) {   // 점수가 max보다 크다면 그 점수가 max다
					max = score[i];
				}
				if(score[i] < min) {   // 점수가 min보다 적다면 그 점수가 min이다
					min = score[i];
				}
			} 
	...
	public void ex6() {
		...
		// flag 상태를 검사
		// flag가 false라면 일치하는 값을 찾기 못했다라는 의미
		// 그런데 flag가 false라면 if 조건식이 false이므로 수행되지 않음
		// -> ! (not연산자 or 부정 논리 연산자) 붙여줌으로써 true로 변경하여
		// {} 안에 코드를 수행하게 끔 함
		if( !flag ) { // 부정 연산자
	...
	public void ex8() {
		...
		// String 클래스의 toString() 메서드가 존재 // 얘가 있어서 주소가 안 나오고 값으로 나옴
	 ...
	 */
		...
		for(int i = 0; i < arr.length; i++) {
			...
			// arr[2] = 'l';
			// arr[3] = 'l';
			// arr[4] = 'o';    => hello 입력하면 [h, e, l, l, o]
		}
		
		System.out.println(Arrays.toString(arr) );
		
		// 2. 문자하나를 입력받아 일치하는 문자가 char 배열에 몇 개 존재하는지 확인
			System.out.print("검색할 문자 입력 : ");
			char ch = sc.next().charAt(0);
			// sc.next() -> String형
			// String.charAt(0) -> String 0번째 인덱스 문자 하나를 추출해서 char 형태로 반환
			// "h" -> 'h'(char형)
